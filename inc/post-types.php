...
function js_custom_init() {
    ...
    if($post_types) {
        foreach ($post_types as $p) {
            ...
            $has_archive = ( isset($p['has_archive']) && $p['has_archive'] ) ? true : false; 
...
function set_custom_cpt_columns($columns) {
    ...
    if($post_type=='properties') {
        unset( $columns['date'] );
        $columns['image'] = __( 'Image', 'red_partners' );
        $columns['property_type'] = __( 'Type', 'red_partners' );
        $columns['broker'] = __( 'Broker', 'red_partners' );
        $columns['date'] = __( 'Date', 'red_partners' );
    }
    return $columns;
}
...
function custom_post_column( $column, $post_id ) {
    ...
    if($post_type=='properties') {
        switch ( $column ) {
            case 'image' :
                $img = get_field('listing_image',$post_id);
                ...
                if($img_src) {
                   $the_photo .= '<img src="'.$img_src.'" alt="" style="width:100%;height:auto" />';
                    ...
                    $the_photo .= '<i class="dashicons dashicons-admin-home" style="font-size:33px;position:relative;top:8px;left:-6px;opacity:0.3;"></i>';
                }
                $the_photo .= '</span>';
                echo $the_photo;
                break;
            case 'broker' :
                $id = get_field('listing_broker',$post_id);
                if($id) {
                ...
                break;
            case 'property_type' :
                $cats = get_the_terms($post_id,'property_types');
                $categories = '';
                if($cats) {
                    $i=1; foreach($cats as $c) {
                        $comma = ($i>1) ? ', ':'';
                        $categories .= $comma . $c->name;
                    $i++; }
                }
                echo $categories;
                break;
